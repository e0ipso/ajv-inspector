...
class AjvInspector {
  ...
  inspect(path, _subSchema) {
    ...
    return this._recurse(pathParts, _subSchema || this.compiled).type;
  }
  ...
  _recurse(pathParts, subSchema, property) {
    ...
    if (typeof path === 'undefined') {
      ...
      subSchema = AjvInspector.dereference(property, subSchema);
      return typeof subSchema === 'function' ? subSchema.schema : subSchema;
    }
    ...
    if (!schemaProperty) {
      const dereferenced = AjvInspector.dereference(property, subSchema);
      if (typeof dereferenced === 'undefined') {
      ...
      this._recurse(pathParts, subSchema, dereferenced);
    }
